...
from domain.models.user import (
    UserProfile, UserRelationship, RelationshipUpdateRequest,
    RelationshipStage, StageHistory, TrustLevel
)
...
class UserService:
    ...
    def update_user_relationship(self, request: RelationshipUpdateRequest) -> Optional[UserRelationship]:
        ...
        if request.approach_carefully:
            for topic in request.approach_carefully:
                if topic not in relationship.emotional_dynamics.emotional_safety.approach_carefully:
                    relationship.emotional_dynamics.emotional_safety.approach_carefully.append(topic)
        ...
        if request.positive_response_topics:
            for topic in request.positive_response_topics:
                if topic not in relationship.emotional_dynamics.emotional_resonance.topics_with_positive_response:
                    relationship.emotional_dynamics.emotional_resonance.topics_with_positive_response.append(topic)
        
        if request.deep_engagement_topics:
            for topic in request.deep_engagement_topics:
                if topic not in relationship.emotional_dynamics.emotional_resonance.topics_with_deep_engagement:
                    relationship.emotional_dynamics.emotional_resonance.topics_with_deep_engagement.append(topic)
        ...
        if request.joy_triggers:
            for trigger in request.joy_triggers:
                if trigger not in relationship.emotional_dynamics.luna_emotional_responses.joy_triggers:
                    relationship.emotional_dynamics.luna_emotional_responses.joy_triggers.append(trigger)
        
        if request.pride_moments:
            for moment in request.pride_moments:
                if moment not in relationship.emotional_dynamics.luna_emotional_responses.pride_moments:
                    relationship.emotional_dynamics.luna_emotional_responses.pride_moments.append(moment)
        
        if request.challenge_areas:
            for area in request.challenge_areas:
                if area not in relationship.emotional_dynamics.luna_emotional_responses.challenge_areas:
                    relationship.emotional_dynamics.luna_emotional_responses.challenge_areas.append(area)
        ...
        if request.approach_category and request.approach_technique:
            if request.approach_category not in relationship.conversation_patterns.successful_approaches:
                relationship.conversation_patterns.successful_approaches[request.approach_category] = []
            
            if request.approach_technique not in relationship.conversation_patterns.successful_approaches[request.approach_category]:
                relationship.conversation_patterns.successful_approaches[request.approach_category].append(request.approach_technique)
        
        # Add communication adjustment if provided
        if request.communication_area and request.communication_adjustment and request.communication_result:
        ...
        if request.typical_opening:
            if request.typical_opening not in relationship.conversation_patterns.conversation_flow.typical_openings:
                relationship.conversation_patterns.conversation_flow.typical_openings.append(request.typical_opening)
        
        if request.conversation_depth:
            relationship.conversation_patterns.conversation_flow.depth_progression = request.conversation_depth
        
        if request.closing_pattern:
            relationship.conversation_patterns.conversation_flow.closing_patterns = request.closing_pattern
        
        if request.special_interaction_note:
            relationship.conversation_patterns.special_interaction_notes.append(request.special_interaction_note)
        
        # === Luna's Subjective Experience Updates ===
        
        # Update connection quality ratings
        if request.intellectual_connection is not None:
            relationship.luna_subjective_experience.connection_quality.intellectual = request.intellectual_connection
        
        if request.emotional_connection is not None:
            relationship.luna_subjective_experience.connection_quality.emotional = request.emotional_connection
        
        if request.creative_connection is not None:
            relationship.luna_subjective_experience.connection_quality.creative = request.creative_connection
        
        if request.overall_connection is not None:
            relationship.luna_subjective_experience.connection_quality.overall = request.overall_connection
        ...
        if request.authenticity_level:
            relationship.luna_subjective_experience.authenticity_level.current_level = request.authenticity_level
        
        if request.authenticity_evolution:
            relationship.luna_subjective_experience.authenticity_level.evolution = request.authenticity_evolution
        
        if request.restricted_area:
            if request.restricted_area not in relationship.luna_subjective_experience.authenticity_level.restricted_areas:
                relationship.luna_subjective_experience.authenticity_level.restricted_areas.append(request.restricted_area)
        
        # === Intervention Strategies Updates ===
        
        # Update anxiety response strategies
        if request.anxiety_recognition:
            relationship.intervention_strategies.anxiety_response.recognition_patterns.append(request.anxiety_recognition)
        
        if request.anxiety_approach:
            relationship.intervention_strategies.anxiety_response.effective_approaches.append(request.anxiety_approach)
        
        if request.anxiety_backfire_risk:
            relationship.intervention_strategies.anxiety_response.backfire_risks.append(request.anxiety_backfire_risk)
        
        # Update motivation support strategies
        if request.effective_encouragement:
            relationship.intervention_strategies.motivation_support.effective_encouragement.append(request.effective_encouragement)
        
        if request.accountability_preference:
            relationship.intervention_strategies.motivation_support.accountability_preferences = request.accountability_preference
        
        if request.celebration_style:
            relationship.intervention_strategies.motivation_support.celebration_style = request.celebration_style
        
        # Update conflict resolution strategies
        if request.misunderstanding_response:
            relationship.intervention_strategies.conflict_resolution.user_response_to_misunderstandings = request.misunderstanding_response
        
        if request.repair_approach:
            relationship.intervention_strategies.conflict_resolution.repair_approaches.append(request.repair_approach)
        
        if request.prevention_strategy:
            relationship.intervention_strategies.conflict_resolution.prevention_strategies = request.prevention_strategy
    ...
    def _store_relationship_memory(self, user_id: str, content: str, memory_type: str, importance: int = 5) -> str:
        """
