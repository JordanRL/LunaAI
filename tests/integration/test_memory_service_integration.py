...
from domain.models.memory import (
    Memory, EpisodicMemory, SemanticMemory, EmotionalMemory, RelationshipMemory,
    MemoryQuery, EpisodicMemoryQuery, SemanticMemoryQuery, 
    EmotionalMemoryQuery, RelationshipMemoryQuery
)
...
elasticsearch_required = pytest.mark.skipif(
    not check_elasticsearch(),
    reason="Elasticsearch is not available at http://localhost:9200"
)
...
class TestMemoryServiceIntegration(unittest.TestCase):
    ...
    def test_memory_caching(self):
        ...
        # Create and store a memory
        memory = EpisodicMemory(
            content="Memory for cache testing",
            importance=7
        )
        memory_id = self.memory_service.store_memory(memory)
