...
from domain.models.memory import (
    EmotionalMemory, EmotionalMemoryQuery, MemoryResult
)
...
- Using emotional thresholds to find memories with specific emotional characteristics""",
    ...
    def handle(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            log(f"Emotional memory query: '{query_text}' | Limit: {limit}", 
            ...
            log(f"Found {memory_count} emotional memories matching '{query_text}'", 
            ...
            if debug_manager.should_debug(DebugLevel.VERBOSE) and memories_list:
                ...
                for i, memory in enumerate(memories_list[:3]):  # Show up to 3 memories
                    ...
                    if trigger:
                        ...
                        trigger_text = ""
                    log(f"  {i+1}. [emotional] {content_preview} (importance: {importance}){trigger_text}", 
                       DebugLevel.VERBOSE)
...
The memory will be stored in Luna's long-term memory for future recall and retrieval.""",
    ...
    def handle(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if memory_id:
                log(f"Created emotional memory: '{content[:50]}...' | Importance: {importance}/10", 
                   DebugLevel.STANDARD, debug_manager.symbols.MEMORY)
